                ...
                with ui.column().classes('lg:col-span-3 w-full gap-6 h-full'):
                    with ui.card().classes(card_style + ' flex-shrink-0'):
                        ...
                        with ui.column().classes('w-full gap-3'):
                            ...
                            ui.button('复制安装命令', icon='content_copy', on_click=copy_cmd).classes('w-full bg-[#172033] border border-slate-600 text-blue-400 shadow-sm hover:bg-[#334155] font-bold align-left')

                            with ui.row().classes('w-full gap-2'):
                                ui.button('分组管理', icon='settings', on_click=lambda: open_unified_group_manager('manage')).classes('flex-1 bg-[#172033] border border-slate-600 text-slate-300 hover:bg-[#334155]')
                                ui.button('排序视图', icon='sort', on_click=open_group_sort_dialog).classes('flex-1 bg-[#172033] border border-slate-600 text-slate-300 hover:bg-[#334155]')

                            ui.button('更新所有探针', icon='system_update_alt', on_click=batch_install_all_probes).classes('w-full bg-orange-900/30 text-orange-400 border border-orange-800 hover:bg-orange-900/50 font-bold align-left')

                    with ui.card().classes('w-full p-6 bg-gradient-to-br from-indigo-900 to-slate-900 text-white rounded-xl shadow-lg relative overflow-hidden group cursor-pointer flex-grow flex flex-col justify-center border border-indigo-500/30').on('click', lambda: ui.navigate.to('/status', new_tab=True)):
